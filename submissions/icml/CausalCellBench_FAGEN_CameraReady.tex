...
Reconstruction accuracy, however, is a property of the marginal distribution of effects, whereas the downstream applications that motivate VCMs---combination prediction, target identification, GRN inference---depend on the joint distribution across genes~\citep{chevalley2025causalbench,vc_causal_2025}: a model can match every per-gene mean and still distort the gene-gene covariance and conditional-independence structure that interventional causal discovery exploits. CausalBench evaluates causal discovery on real Perturb-seq~\citep{chevalley2025causalbench} but does not test whether VCM predictions can substitute for it; a NeurIPS 2025 perspective called for causal-recovery evaluation of VCMs but did not implement one~\citep{vc_causal_2025}. Adjacent work surfaces pieces---mode collapse and weighted losses~\citep{mejia2025diversity}, metric calibration~\citep{miller2025calibrated}, and the inspre sparse-inverse causal-discovery algorithm~\citep{brown2025inspre}---but none integrates VCM predictions into a downstream causal-discovery pipeline to test substitutability directly.
...
\item \textbf{A diagnostic decomposition of the substitutability gap.} The gap separates into a magnitude marginal (mode collapse and effect-size inflation, two views of one failure) and a joint-distribution residual. Post-hoc quantile-matching calibration~\citep{bolstad2003,reinhard2001} corrects the magnitude marginal exactly, yet under a held-out perturbation split no calibration ordering closes the causal gap. We make the decomposition precise: conditional independence is invariant under per-gene monotone calibration (Proposition~\ref{prop:ci-invariance}), so marginal rescaling provably cannot close the gap, identifying the residual as the off-diagonal joint (precision-support) structure and motivating a training-time remedy.
\end{enumerate}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Related Work}
\label{sec:related}
A 2024--2025 literature converges on deep learning not yet beating simple baselines on per-gene reconstruction~\citep{ahlmanneltze2025,wong2025simple,csendes2025benchmark,wei2025benchmark,vinas2025systema,bendidi2024pca,wenteler2025perteval,zhu2025garm,li2025systematic,mejia2025diversity}, standardized by PerturBench~\citep{wu2024perturbench} and the Virtual Cell Challenge~\citep{roohani2025vcc}; none evaluate downstream causal recovery. A parallel literature builds causal discovery on \emph{real} Perturb-seq---GIES~\citep{hauser2012gies}, inspre~\citep{brown2025inspre} (the backends we reuse), BEELINE~\citep{pratapa2019beeline}, Scribe~\citep{qiu2020scribe}, CASCADE~\citep{cao2025cascade}, IV methods~\citep{park2026confounder}, CausalBench~\citep{chevalley2025causalbench}, with \citet{kernfeld2024fdr} cautioning transcriptome data alone cannot control false discoveries; PerturbCausal is downstream, asking whether \emph{predicted} data can substitute. This is orthogonal to GRN-conditioned simulators (GRouNdGAN~\citep{zinati2024groundgan}), the data-pathology axis of \citet{fernandez2026grnbreak}, the taxonomy of \citet{vc_causal_2025}, the featurizer use of \citet{wu2025cdn}, and retraining-based mode-collapse losses~\citep{mejia2025diversity}; our edge-set-rank causal F1 against a fixed real-data GIES ground truth is complementary to, not contradicted by, the dynamic-range-fraction results of \citet{miller2025calibrated} (full per-category discussion in \S\ref{app:related-extended}).
...
A virtual cell model $f$ maps a perturbation target $p$ and a control state to a predicted profile $\hat{x}^{(p)}\in\mathbb{R}^{G}$, and a causal-discovery algorithm $\mathcal{A}$ maps interventional samples $\{(\hat{x}^{(p)},p)\}_{p\in\mathcal{P}}$ to a directed edge set $\hat{E}_{\mathcal{A}}$. The substitutability question is whether $\hat{E}_{\mathcal{A}}$ recovered from \emph{predicted} data approximates $E^{\star}_{\mathcal{A}}=\mathcal{A}(\{x^{(p)}\})$ recovered from \emph{real} Perturb-seq under the same $\mathcal{A}$; holding $\mathcal{A}$ fixed and varying only the data source attributes any gap to the predictor $f$. Anchoring ground truth to the same backend on real data isolates the predictor's contribution, at the cost that edge-set $F_1$ is comparable across predictors but not across backends, so every $F_1$ is reported within a backend. The linear predictor is a primary comparator, not a floor: the question is asked symmetrically of deep and linear models, following the finding that linear baselines are competitive on per-gene prediction~\citep{ahlmanneltze2025}.

\subsection{Datasets}
We use three Perturb-seq datasets spanning in-distribution, cross-context, and cross-paradigm generalization: Replogle K562~\citep{replogle2022} (CRISPRi, primary; 1{,}861 eligible targets), Replogle RPE1~\citep{peidli2024scperturb} (CRISPRi, cross-context), and Norman 2019~\citep{norman2019} (CRISPRa, cross-paradigm; 102 single-gene targets); the primary K562 evaluation uses a 200-gene subset that fills exactly ten 20-gene GIES partitions, and the scale sweep $N\in\{50,75,100,125,150,175,200\}$ confirms rankings are stable across subset size (full dataset statistics and gene-subset construction in \S\ref{app:extended-methods}).
...
\emph{Overlay sampling} converts predicted means into cell matrices that preserve gene-gene covariance (independent negative-binomial sampling instead collapses GIES recovery below 2\% of the real-data ceiling): control cells are bootstrap-sampled and scaled by the predicted fold change with Poisson noise, matching the CausalBench protocol~\citep{chevalley2025causalbench}. On K562, GIES, PC, and NOTEARS consume overlay cells; for the cross-dataset evaluation PC, NOTEARS, and inspre consume the square average causal effect (ACE) matrix $\mathrm{ACE}_{ij}=\mathbb{E}[X_j\mid\mathrm{do}(X_i)]-\mathbb{E}[X_j\mid\mathrm{ctrl}]$ on the canonical perturbed-gene set, so the node set is identical across predictors and only the predicted intervention effect varies (overlay-sampling sensitivity sweep in \S\ref{app:extended-methods}).
...
\subsection{Chance reference and metric}
\label{sec:chance}
The primary chance reference is a \emph{permutation null} computed per backend: each column of the real-data ACE is independently shuffled across perturbations, removing cross-gene joint structure while preserving each gene's marginal effect-magnitude distribution. The backend is run on $K$ such nulls ($K=30$, except $K=24$ for PC on Norman), giving a mean and a $[2.5,97.5]$ percentile band against $E^{\star}_{\mathcal{A}}$; a predictor's $F_1$ is read as \emph{above} the band (signal beyond a structure-destroyed input), \emph{inside} (at chance), or \emph{below} (worse than the shuffled null). A secondary density-matched random-edge floor is reported only to confirm a predictor is not emitting a trivially small or large edge set (analytic form in \S\ref{app:extended-methods}). The primary metric is edge-set $F_1$ against the per-backend real-data ground truth (precision, recall, AUPRC, SHD, and multi-hop credit at $k\in\{2,3\}$ secondary).

\subsection{Statistical testing and calibration}
K562 headline numbers are across-seed means with 95\% $t$-CIs over $n\in\{3,5,20\}$ seeds; deep-vs-linear equivalence is assessed by two one-sided tests (TOST) with margin $\Delta = 0.045$ (the technical-duplicate noise floor), and cross-dataset cells are single-seed estimates read against the per-backend permutation band of \S\ref{sec:chance}. Post-hoc quantile-matching calibration~\citep{bolstad2003,reinhard2001} maps the predicted $|\log_2\mathrm{FC}|$ distribution onto the real-data distribution, with two joint-structure extensions evaluated under a 160/40-perturbation split (full statistical-testing protocol, implementation specifics, and the calibration algorithm in \S\ref{app:extended-methods} and \S\ref{app:calib-sweep}).
...
We evaluate all conditions across five random seeds (42, 123, 456, 789, 1024) with $t$-distribution 95\% CIs and paired permutation tests ($10{,}000$ permutations). Against 1{,}197--1{,}230 ground-truth edges per seed, all four predictive methods fall within a tight band (ElasticNet F1 $= 0.426$, CPA $= 0.412$, GEARS $= 0.406$, Geneformer $= 0.425$, overlay-resampled real $= 0.422$; Table~\ref{tab:main}, Figure~\ref{fig:main}) with \textbf{no pairwise difference significant at $p<0.05$}; ElasticNet (0.426) and Geneformer (0.425) are statistically tied. A two-one-sided-tests (TOST) analysis establishes equivalence at the technical-duplicate margin ($\Delta = 0.045$ F1): the ElasticNet$-$deep paired differences lie within $\pm\Delta$ for all three deep models (90\% CI: CPA $+0.014\,[-0.001, +0.029]$, GEARS $+0.020\,[+0.005, +0.035]$, Geneformer $+0.0002\,[-0.012, +0.013]$). The observational baseline GRNBoost2 falls to F1 $0.098$ $[0.094, 0.103]$, disjoint from every predictive method by $\geq 0.27$ F1 (gap significant at $n=20$, $p<10^{-4}$); the random-bootstrap floor is F1 $\approx 0.04$ and the technical-duplicate noise floor $0.045 \pm 0.003$ (\S\ref{app:noisefloor}). The headline is not that ElasticNet outperforms deep models---the previous single-seed claim does not survive multi-seed evaluation---but that a 104M-parameter pretrained transformer, a GNN with Gene Ontology priors, and an adversarial VAE all achieve causal-recovery F1 statistically indistinguishable from a sparse linear regression with $\approx\!200^2$ free coefficients.

\begin{table*}[t]
\caption{Causal recovery on K562 at $N{=}200$ genes against real-data GIES ground truth. Multi-seed mean F1, precision, recall, and edge count across $n=5$ random seeds (42, 123, 456, 789, 1024) with 95\% $t$-distribution CIs. ``Tech.\ duplicate'' is F1 between two GIES recoveries on disjoint random halves of real cells (mean of 3 splits). Pairwise permutation tests find no significant difference at $p<0.05$ among the four predictive methods. The ElasticNet--GRNBoost2 gap is significant at $n=20$ ($p<10^{-4}$ paired permutation), while the $n=5$ deep-model comparisons have a minimum attainable two-sided $p$ of $2/2^5 = 0.0625$, not yet certifiable despite the same effect-size gap.}
\label{tab:main}
\vskip 0.1in
\centering
\small
...
Real K562 responses are sparse (2.7\% of (perturbation, gene) effects exceed $|\log_2 \mathrm{FC}| = 0.5$; leading singular vector captures 56.8\% of variance), yet all three deep models inflate that fraction to 63.2\% (GEARS), 63.4\% (CPA), 63.5\% (Geneformer)---a 23-fold inflation identical across architectures to within 0.3 percentage points (Figure~\ref{fig:inflation}), with mean per-perturbation $L_1$ norm roughly tenfold larger than empirical (real $22.8$; GEARS $226.9$, CPA $226.6$, Geneformer $227.4$). The control-regime covariance is preserved well (correlation of correlation matrices: GEARS = 0.967, CPA = 0.996, Geneformer = 0.996), so the models learn baseline gene-gene relationships but fail to predict sparse, structured deviations upon perturbation.

\paragraph{Mode collapse and inflation are one failure.} The pooled 23-fold inflation is the pooled view of the same homogenization reported as mode collapse \citep{mejia2025diversity,ahlmanneltze2025}: per-perturbation $\Delta$-variance ratios are below 1 for all three deep models (median 0.461 GEARS, 0.432 CPA, 0.008 Geneformer) and inter-perturbation shift-cosine is 0.845 in CPA vs.\ 0.084 in real---a single dense response pattern across perturbations that, pooled over off-target genes, reads as inflation (full reconciliation in \S\ref{app:reconciliation}).

\paragraph{Technical-duplicate noise floor.} Two GIES recoveries on disjoint random halves of real K562 cells agree at only $F_1 = 0.045 \pm 0.003$ (Table~\ref{tab:main}), so real-cell sampling variance dominates GIES at $N{=}200$ and all evaluated methods operate roughly an order of magnitude above this raw cell-split floor (\S\ref{app:noisefloor}).

\subsection{Inspre signal probe}
\label{sec:inspre-probe}
The inspre algorithm~\citep{brown2025inspre}, applied to the ACE matrix without partitioning, recovers 12{,}280 edges from real Perturb-seq, exactly 12{,}280 from ElasticNet, 6{,}234 from GEARS, and zero from both CPA and Geneformer at the test-error-optimal regularization (Figure~\ref{fig:inspre}). The full $\lambda$-path validation (\S\ref{app:inspre-path}) shows the zero result is binary at the cross-validated $\lambda$ but continuous across the path, with CPA vanilla the only condition for which the solver fails to traverse a valid $\lambda$ path.

\subsection{Cross-context replication on RPE1}
\label{sec:rpe1}
GEARS and CPA retrained from scratch on RPE1 (8{,}749 genes, 196 targets) recover no more of the real causal graph than the linear baseline or chance: against the real-data RPE1 ground truth (1{,}137 edges, seed 42), GEARS F1 $= 0.414$, CPA $= 0.450$, ElasticNet $= 0.437$, random-edge floor $0.436$, and the perfect-real-means overlay upper bound itself reaches only $0.418$. The substitutability gap therefore persists under a change of cellular context (single-seed, qualitative claim; full prose, $N{=}200$ structural comparison, scale sweep, and reversed-direction analyses in \S\ref{app:rpe1}, \S\ref{app:scale-sweep}, \S\ref{app:reversed}).
...
The chance comparison, read in three states against the permutation band, holds the strict ``at chance'' characterization for GIES, NOTEARS, and inspre-on-RPE1 (deep models at or below floor), with two above-floor exceptions that neither exceeds the linear comparator nor reaches substitutable quality: PC on RPE1 ($F_1 = 0.078$ vs.\ a $0.017$ floor, but PC is observational and discards intervention labels, so this is preserved baseline co-expression) and GEARS under inspre on Norman ($0.344$ against a $0.329$ band edge, a single-seed margin). The cell-by-cell reading---including why inspre's high raw $F_1$ (GEARS $0.416$, CPA $0.440$ on RPE1) sits inside an equally high $0.526$ $[0.406, 0.583]$ band---is in \S\ref{app:extended-discussion}.

\begin{table*}[t]
\caption{Backend robustness on RPE1 (cross-context) and Norman (cross-paradigm). Per-backend edge-set $F_1$ for the two trained deep VCMs and the linear comparator (ENet) against each backend's own real-data ground truth; the chance floor is the permutation-null mean with $[2.5, 97.5]$ band (\S\ref{sec:chance}). Verdicts: $^{\uparrow}$ above band, $^{=}$ inside (at chance), $^{\downarrow}$ below. No deep model exceeds ENet in any cell. ENet under PC on RPE1 is omitted: PC does not converge on the dense ElasticNet ACE within the compute budget.}
\label{tab:backend}
\vskip 0.1in
\centering
\small
...
\end{center}
\caption{Deep VCM causal-recovery $F_1$ against the per-backend chance floor on RPE1 and Norman. Each row is one (dataset, backend) cell; the grey band is the permutation-null $[2.5, 97.5]$ chance interval ($K{=}30$ permutations, $24$ for PC on Norman). Markers are GEARS (circle), CPA (square), and the ElasticNet comparator (open triangle). Both deep models land inside or below the band in four of six cells; the two above-band points (PC on RPE1 at $F_1 = 0.078$; GEARS under inspre on Norman, a single-seed margin) neither exceed the linear comparator nor reach substitutable quality. $n_{\text{gt}}$ ground-truth edges per cell range from 67 to 1{,}327.}
\label{fig:backend}
\vskip -0.1in
\end{figure}

\subsection{Calibration corrects magnitude but not held-out causal F1}
In-domain quantile-matching calibration corrects the magnitude distribution exactly but moves downstream causal F1 only marginally (CPA $0.389 \to 0.405$, GEARS $0.406 \to 0.413$, Geneformer $0.432 \to 0.423$; \S\ref{app:calib-indomain}, Table~\ref{tab:cal}, Figure~\ref{fig:calibration}), and no calibrated deep model surpasses ElasticNet (F1 = 0.425). Under an honest held-out perturbation split, no calibration ordering beats the uncalibrated baseline on any model (GEARS $0.303$ vs.\ $0.290$, CPA $0.309$ vs.\ $0.289$, Geneformer $0.319$ vs.\ $0.287$; \S\ref{app:calib-sweep}), locating the residual in joint structure rather than the magnitude marginal.
...
\end{center}
\caption{Causal recovery on K562 at $N{=}200$ genes against the real-data GIES ground truth. Reference rows (real Perturb-seq, overlay-resampled real, ElasticNet, GRNBoost2) are aggregated over $n{=}20$ seeds; the deep VCMs (GEARS, CPA, Geneformer) over the $n{=}5$ seeds with cached predictions (42, 123, 456, 789, 1024). Bars are across-seed mean F1; error bars are 95\% $t$-distribution CIs. The dashed red line marks the technical-duplicate noise floor (mean F1 = 0.045 between two GIES recoveries on disjoint random halves of real cells). Wong colorblind-safe palette.}
\label{fig:main}
\vskip -0.1in
\end{figure}
...
\end{center}
\caption{Mode-collapse vs.\ inflation reconciliation diagnostic on K562 ($N{=}200$, 193 perturbations). (a)~Per-perturbation $\Delta$-variance ratios (dashed line: ratio = 1); all deep models compress, Geneformer by $\sim$two orders of magnitude. (b)~Inter-perturbation cosine similarity (whiskers $p10$--$p90$); CPA produces nearly the same response across perturbations (0.84 vs.\ 0.08 real). (c)~Mean per-perturbation fraction $|\log_2$FC$|>\tau$ vs.\ threshold $\tau$; deep models track real or fall below per-perturbation yet exceed real at the full-transcriptome pool. Full panel descriptions in \S\ref{app:reconciliation}.}
\label{fig:inflation}
\vskip -0.1in
\end{figure*}
...
\paragraph{Mechanism: magnitude and causal recovery are dissociated.}
The shared failure is objective-driven, not architecture-driven, and lies in joint rather than marginal structure. Retraining CPA with a Gaussian (per-gene MSE) reconstruction loss instead of its default Negative Binomial \emph{increases} per-perturbation variance compression (median $\Delta$-variance ratio 0.43 $\to$ 0.18) yet \emph{raises} causal F1 (0.389 $\to$ 0.414 at seed 42): more aggressively compressed predictions recover \emph{more} of the real graph, so the gap is neither in the reconstruction-loss form nor the magnitude marginal but in the joint covariance/conditional-independence structure GIES extracts (full mechanism analysis in \S\ref{app:j2}). This motivates a training-time covariance penalty $\lambda\,\lVert \mathrm{Cov}(\hat\delta) - \mathrm{Cov}(\delta_{\text{real}})\rVert_F^2$, released as a unit-tested module and tested by a no-GPU proxy: a learned joint head on CPA's shifts under a held-out 160/40 split (\S\ref{app:j2}) raises held-out causal $F_1$ from vanilla CPA's $0.424 \pm 0.025$ to $0.535 \pm 0.015$ ($\Delta = +0.111 \pm 0.034$), the first direct confirmation the gap is reachable by a joint operation that generalizes to unseen perturbations. The covariance-Frobenius term itself does not help: a pre-registered sweep $\lambda\in\{0,0.1,0.3,1,3,10,30,100\}$ (validation-selected, sealed test touched once) finds the best weight ($\lambda{=}100$) gains only $+0.003 \pm 0.035$ test $F_1$ over recon-only (3 of 5 seeds) and fails a multi-seed gate---the gain comes from learning a generalizing joint map, not the covariance term. End-to-end retraining scored against the alt-loss $F_1$ of 0.414 is the natural next experiment.

A six-variant CPA ablation confirms the residual gap is not localizable to any one architectural component (causal-F1 range $0.387$--$0.418$, within vanilla CPA's 95\% CI; \S\ref{app:cpa_ablation}).

\paragraph{Why does ElasticNet match (not beat) deep models?}
ElasticNet F1 ($0.416$) and the three deep models ($0.405$--$0.425$) fall within overlapping 95\% CIs: a sparse linear regression with $\approx\!200^2$ free coefficients trained only on control cells matches, but does not outperform, a 104M-parameter pretrained transformer. We attribute this to inductive-bias matching: $L_1$ regularization biases ElasticNet toward sparsity~\citep{zou2005elasticnet}, matching the empirical sparsity of real perturbation responses (38.6\% of K562 effects near zero, 56.8\% of variance in the leading singular vector), a bias the deep models' training objectives lack.

\paragraph{Held-out calibration null and the identifiability argument.}
\label{sec:calib-sweep}
Extending Bolstad--Reinhard quantile matching with two joint-structure modes (covariance preservation, sparsity injection) and evaluating all nine orderings under an honest 160/40-perturbation train/test split, \textbf{no calibration ordering exceeds the uncalibrated baseline on any model}: GEARS vanilla $0.303$ vs.\ best calibrated $0.290$; CPA $0.309$ vs.\ $0.289$; Geneformer $0.319$ vs.\ $0.287$ (full sweep and table in \S\ref{app:calib-sweep}). Post-hoc magnitude---and joint-structure---calibration thus fails on held-out perturbations: the substitutability gap is not in the magnitude marginal but in the joint structure, and closing it requires a training-time intervention rather than post-hoc rescaling. This null is not an artifact of our particular calibrator; it is forced by an invariance.
...
Marginal rescaling therefore provably cannot close the gap (proof, corollary, and the finite-sample reading for the score-based and covariance/precision backends in \S\ref{app:identifiability}): the residual lives in the off-diagonal joint (precision-support) structure, motivating a training-time rather than post-hoc remedy.

\paragraph{STATE non-replication.}
\label{sec:state-multi-seed}
Arc Institute's 2025 STATE foundation model shows an apparent seed-42 advantage ($F_1 = 0.462$) that does not replicate: the three per-seed values are $[0.462, 0.041, 0.048]$, two of three near $0.04$ and well below the ElasticNet mean of $0.418$, so the substitutability gap holds for the 2025 foundation model as well (pipeline, framework-version fixes, and the 1{,}200-cell sampling control in \S\ref{app:state_methods}).

\paragraph{Limitations.}
\label{sec:limitations}
The central limitation is the ground truth: Track A (real-data GIES) is internally consistent but does not establish biological correctness, and a multi-seed external check against curated TRRUST is a genuine null ($0.83 \pm 0.70\times$ a matched random-edge floor; \S\ref{app:trrust})---at this scale GIES is too cell-sampling-sensitive (technical-duplicate $F_1 = 0.045$) to validate against a sparse curated network~\citep{kernfeld2024fdr}. Geneformer's pretraining includes Replogle 2022 (partial memorization), cross-dataset cells are single-seed, and primary results are at $N=200$. The full discussion is in \S\ref{app:limitations}.
...
The result holds across the conditions tested: GEARS and CPA retrained on RPE1 and Norman never exceed the linear comparator under any backend (largest cross-dataset $F_1 = 0.440$, inside its chance band), and STATE's single-seed advantage does not replicate ($F_1 = 0.462, 0.041, 0.048$). Single-seed cross-dataset cells make this consistency across conditions rather than proven invariance.

The failure decomposes into a magnitude marginal (mode collapse and effect-size inflation, two views of one homogenized response, partially correctable post-hoc by Bolstad--Reinhard quantile matching) and a joint-distribution residual in gene-gene covariance and edge orientation that distributional calibration cannot reach. Closing the gap therefore calls for a training-time intervention, not post-hoc rescaling. The benchmark, calibration diagnostic, multi-seed pipeline, and reproducibility infrastructure are released as the open-source \texttt{PerturbCausal} Python package.
...
\subsection{Algorithmic robustness and inspre lambda-path discussion}
...
\subsection{RPE1 cross-context replication and structural comparison}
\label{app:rpe1}
...
\subsection{Why marginal calibration cannot close the gap: proof and finite-sample reading}
\label{app:identifiability}
The held-out calibration null of \S\ref{sec:calib-sweep}---no quantile-matching ordering beats the uncalibrated predictor---is not an artifact of our particular calibrator. It is forced by the invariance stated as Proposition~\ref{prop:ci-invariance}, proved here.

\begin{proof}[Proof of Proposition~\ref{prop:ci-invariance}]
