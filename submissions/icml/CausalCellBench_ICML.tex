...
\bottomrule
\end{tabular}
\caption{No-GPU covariance-regularizer proxy: a learned joint transform on CPA shifts, mean $F_1\pm$SD over five 160/40 splits. A learned generalizing transform recovers $+0.111$ F1 over vanilla CPA where post-hoc marginal calibration (\S\ref{sec:calib-sweep}) recovered nothing; the explicit covariance-Frobenius penalty does not robustly improve on plain reconstruction at $\lambda{=}1$ (cov-reg$-$recon $=-0.004\pm0.050$, positive in 2/5 splits). Reproducible via \texttt{scripts/j2\_covariance\_regularizer\_experiment.py}.}
\label{tab:j2}
\end{table}
