...
Replogle RPE1  & ENet, GEARS, CPA, GRNBoost2, overlay-real                  & GIES                       & 1     & cross-context (CRISPRi, RPE1) \\
Norman 2019    & ENet, GEARS, CPA, GRNBoost2, overlay-real                  & GIES                       & 1     & cross-paradigm (CRISPRa, K562) \\
\bottomrule
\end{tabular}
\caption{Scope-of-empirical-claims table. Deep-model substitutability is tested with two architecturally distinct trained VCMs (GEARS, CPA) on the cross-context (RPE1) and cross-paradigm (Norman) datasets in addition to K562; both recover no more than chance on both. Geneformer multi-dataset is omitted (its prediction is a similarity heuristic, not learned ISP expression, \S\ref{sec:limitations}).}
\label{tab:scope}
\end{table}
...
To test whether the substitutability conclusion generalizes across perturbation modality (CRISPRa rather than CRISPRi) and laboratory (Norman et al.\ 2019 vs.\ Replogle et al.\ 2022), we evaluated the full method panel on Norman 2019 K562 data, restricted to $N{=}102$ single-gene perturbation-eligible targets (combinatorial conditions dropped so the on-target-edge metric matches K562/RPE1) on an 8{,}028-gene HVG background chosen for CPA-training tractability. We retrained both GEARS and CPA from scratch on this Norman subset and pushed their predictions through the identical GIES pipeline. Against the real-data Norman ground truth (209 edges), GEARS recovers F1 $= 0.211$ [0.176, 0.248] and CPA F1 $= 0.182$ [0.155, 0.212], both indistinguishable from the random-edge floor (F1 $= 0.179$ [0.147, 0.209]) and the ElasticNet baseline (F1 $= 0.190$ [0.158, 0.223]). The substitutability gap therefore persists under a change of perturbation paradigm: deep VCMs retrained on CRISPRa data recover no more of the real causal graph than chance, while the sparse ElasticNet baseline remains competitive --- confirming the conclusion is neither Replogle-K562-specific nor an artifact of the CRISPRi modality.
