...
\midrule
Replogle K562  & ENet, CPA, GEARS, Geneformer, STATE, GRNBoost2, overlay-real & GIES, inspre, PC, NOTEARS & 3--20 & primary; STATE 3-seed 320-cell subset \\
Replogle RPE1  & ENet, CPA, GRNBoost2, overlay-real                         & GIES                       & 1     & cross-context (CRISPRi, RPE1) \\
Norman 2019    & ENet, CPA, GRNBoost2, overlay-real                         & GIES                       & 1     & cross-paradigm (CRISPRa, K562) \\
\bottomrule
\end{tabular}
\caption{Scope-of-empirical-claims table. Deep-model substitutability is now tested with CPA on the cross-context (RPE1) and cross-paradigm (Norman) datasets in addition to K562; GEARS, Geneformer, and STATE multi-seed on RPE1/Norman are reported as in-progress in \S\ref{sec:limitations}.}
\label{tab:scope}
\end{table}
...
To test whether the substitutability conclusion generalizes across perturbation modality (CRISPRa rather than CRISPRi) and laboratory (Norman et al.\ 2019 vs.\ Replogle et al.\ 2022), we evaluated the full method panel on Norman 2019 K562 data, restricted to $N{=}102$ single-gene perturbation-eligible targets (combinatorial conditions dropped so the on-target-edge metric matches K562/RPE1) on an 8{,}028-gene HVG background chosen for CPA-training tractability. We retrained CPA directly on this Norman subset and pushed its predictions through the identical GIES pipeline. Against the real-data Norman ground truth (209 edges), CPA recovers F1 $= 0.182$ [0.158, 0.208], indistinguishable from the random-edge floor (F1 $= 0.180$ [0.150, 0.207]) and below the ElasticNet baseline (F1 $= 0.192$) and overlay-resampled real (F1 $= 0.226$). The substitutability gap therefore persists under a change of perturbation paradigm: a deep VCM retrained on CRISPRa data recovers no more of the real causal graph than chance, while the sparse ElasticNet baseline remains competitive --- confirming the conclusion is neither Replogle-K562-specific nor an artifact of the CRISPRi modality.
...
The structural properties of RPE1 perturbation responses differ from K562: leading singular variance 34.5\% (vs.\ 56.8\%), fraction near-zero 15.4\% (vs.\ 38.6\%), fraction large 17.3\% (vs.\ 2.7\%). We retrained CPA directly on RPE1 (8{,}749 genes, 196 perturbation-eligible targets) and pushed its predictions through the identical GIES pipeline. Against the real-data RPE1 ground truth (1{,}137 edges), CPA recovers F1 $= 0.360$ [0.345, 0.378], statistically indistinguishable from the random-edge floor (F1 $= 0.348$ [0.328, 0.364]) and the ElasticNet baseline (F1 $= 0.340$ [0.322, 0.356]); bootstrap confidence intervals overlap fully. The substitutability gap therefore persists under a change of cellular context: a deep VCM retrained on RPE1 recovers no more of the real causal graph than a sparse linear baseline or chance.
...
\begin{tabular}{lrrr}
\toprule
\textbf{Model} & \textbf{Vanilla F1} & \textbf{Best calibrated F1} & \textbf{Best ordering} \\
\midrule
GEARS       & 0.303 & 0.289 & \texttt{s} (sparsity-only) \\
CPA         & 0.309 & 0.302 & \texttt{scq} (sparsity$\to$covariance$\to$quantile) \\
Geneformer  & 0.319 & 0.290 & \texttt{s} (sparsity-only) \\
\bottomrule
\end{tabular}
\caption{Phase~4 calibration sweep on K562 test split (40 of 200 perturbations held out). Per-model best F1 over nine ordering choices vs.\ the uncalibrated baseline on the same test split. No ordering exceeds vanilla; the substitutability gap does not close under post-hoc magnitude or joint-structure calibration.}
...
Re-running the identical SE-600M $\to$ ST-SE-Replogle $\to$ overlay $\to$ GIES pipeline at two further seeds (320-cell subsets resampled with seeds 123 and 456) yields $F_1 = 0.040$ and $F_1 = 0.051$ --- an order of magnitude below the seed-42 value. The three-seed mean is $F_1 = 0.184$ with a 95\% confidence interval of $[-0.088, 0.457]$ that spans zero and sits below the ElasticNet baseline. The seed-42 ``narrowing'' was a fortunate draw of the 1-cell-per-perturbation sampling rather than a robust property of the model. We therefore cannot conclude that STATE exceeds the linear baseline: its apparent advantage is within sampling noise, and the central substitutability gap holds for the 2025 foundation model as well. The instability is itself diagnostic --- a single perturbed cell per gene is too few to estimate the STATE-predicted perturbation mean stably, so a definitive STATE assessment requires the higher-cell-count embedding that the $\sim$28\,s/cell SE-600M CPU cost placed outside the 6\,h Modal budget (Appendix~\ref{app:state_methods}). Absolute $F_1$ at every seed remains below $0.5$.

\paragraph{Limitations.}
\label{sec:limitations}
Six limitations are noted. \textit{Ground truth.} Track A ground truth (real-data GIES) is internally consistent but does not establish biological correctness; a Track B evaluation against the literature-curated TRRUST v2 database~\citep{han2018trrust} is implemented in the released code but reported as a robustness check (Spearman concordance with Track A model rankings rather than absolute F1 against TRRUST). \textit{Pretraining contamination.} Geneformer V2's Genecorpus pretraining includes Replogle 2022, so its Replogle F1 is partly memorization. We quantify this with a cell-line-holdout comparison (Geneformer F1 on K562 vs RPE1) rather than a temporally held-out post-pretraining dataset, which would require a 2024+ public Perturb-seq atlas on a Geneformer-corpus-excluded human cell type; no such dataset is currently available with $\geq 50$ perturbation-eligible genes. \textit{Seed budget.} Multi-seed CIs reported here use $n \leq 20$ seeds for K562 and $n=1$ for RPE1 and Norman; the qualitative finding that pairwise differences among predictive methods are not significant is robust to seed count, but minimum detectable F1 differences at $n=20$ are approximately 0.011. \textit{Cross-dataset deep-model scope.} CPA was retrained from scratch and evaluated through the full GIES pipeline on both RPE1 (cross-context) and Norman (cross-paradigm), recovering no more of the real causal graph than chance in either (\S\ref{sec:scope}); the cross-context and cross-paradigm claims therefore no longer rest on the ElasticNet baseline alone. GEARS and Geneformer on RPE1/Norman, and STATE multi-seed, add redundant deep-model confirmation and are compute-bound in progress. \textit{Model roster.} GEARS, CPA, and Geneformer were chosen for architectural diversity at the time of pipeline construction; they predate STATE~\citep{adduri2025state,arc2025state} and other 2025--2026 perturbation models. Recent benchmarks~\citep{wong2025simple,csendes2025benchmark,wei2025benchmark,vinas2025systema} cover additional models on the per-gene prediction task but not on the causal-recovery task evaluated here. \textit{Gene-set scale.} Primary results are at $N=200$ for compute-tractability of GIES partitioning; scale-sweep numbers extend to $N=200$ along seven points and inspre extends scaling to $\sim$1000 genes, but a head-to-head deep-model evaluation at $N=500$--$1000$ is not yet completed.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Conclusion}
\label{sec:conclusion}

PerturbCausal is the first benchmark testing whether virtual cell model predicted Perturb-seq data can substitute for real Perturb-seq in downstream causal gene regulatory network recovery. Across one primary dataset (Replogle K562) with up to 20 random seeds, five predictive methods (ElasticNet, CPA, GEARS, Geneformer, STATE), and four causal-discovery backends (GIES, inspre, PC, NOTEARS), no deep virtual cell model significantly exceeds a sparse linear regression baseline on causal-recovery F1; STATE's single-seed apparent narrowing does not replicate across seeds (three-seed $F_1 = 0.184$, CI spanning zero), and CPA retrained on a second cell context (RPE1) and a second perturbation paradigm (Norman CRISPRa) likewise recovers no more than chance. The substitutability failure decomposes into a magnitude marginal (mode collapse + effect-size inflation, the two equivalent views identified by \citet{mejia2025diversity} and our diagnostic, partially correctable post-hoc via Bolstad--Reinhard quantile matching) and a joint-distribution residual (covariance and edge-orientation structure that distributional calibration cannot reach); the benchmark, three-mode calibration diagnostic, multi-seed pipeline, and reproducibility infrastructure are released as the open-source \texttt{PerturbCausal} Python package.
