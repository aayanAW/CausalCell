...
\begin{abstract}
Virtual cell models (VCMs) predict transcriptional responses to genetic perturbations and are promoted as in silico substitutes for CRISPR screens; \citet{ahlmanneltze2025} showed they fail to beat linear baselines at per-gene prediction. We ask whether predicted Perturb-seq can substitute for real Perturb-seq as the input to causal gene-regulatory-network recovery, holding the discovery pipeline fixed and varying only the data source. The verdict is negative---no VCM reaches real-data quality---but the operative mechanism is not what abstract metrics suggest. A cell-split design on K562 (ground truth and effect estimand on disjoint cell halves, ten seeds, permutation null) yields three findings. First, the apparent positive control is differential-expression test--retest reliability, not causal discovery: on a total-effect (reachability) ground truth real clears the null two-to-threefold, yet a DE $t$-test recovers that same graph better ($F_1$ 0.60--0.75) than the causal average-causal-effect ($|\mathrm{ACE}|$) estimand (0.30--0.43). Second, direct causal-edge recovery is near-unidentifiable at achievable SNR: against a direct-edge ground truth, real $F_1$ is only 0.03--0.12, barely above a permutation null. Third, VCMs fail because their means are wrong (mode collapse; $23\times$ effect-size inflation), not because causal recovery needs joint structure the mean overlay hides---a perfect-mean emitter ties real ($F_1$ 0.42 vs.\ 0.43), so the recoverable signal is the means, while mode-collapsed CPA, GEARS, and Geneformer score three-to-fivefold lower (0.08--0.18) even here. We prove the mean-overlay evaluation is invariant to joint structure, but frame this only as a corollary of published covariance-fidelity theory \citep{riasat2026marginals,rubbi2026ggeval}, not the mechanism of failure here. We release the benchmark, falsification battery, and reproducibility code as an open-source Python package.
\end{abstract}
...
\begin{enumerate}
\item \textbf{A substitutability benchmark for causal-structure recovery.} We ask whether VCM-predicted Perturb-seq can substitute for \emph{real} Perturb-seq as the \emph{input} to causal gene-regulatory-network recovery: the discovery algorithm is held fixed and only the data source varies, so any gap is attributed to the predictor, and the ground truth is the same algorithm's output on real data. Four VCMs (GEARS, CPA, Geneformer, and the 2025 Arc Institute STATE model~\citep{roohani2024gears,lotfollahi2023cpa,theodoris2023geneformer,adduri2025state}) and a sparse linear comparator are passed through four discovery backends (GIES, inspre, PC, NOTEARS~\citep{hauser2012gies,brown2025inspre}) on three Perturb-seq datasets spanning cell context and modality~\citep{replogle2022,norman2019}. This measures utility for causal-structure recovery, distinct from the concurrent utility-for-hit-discovery benchmark of \citet{bereket2026perturbhd}: hit ranking and causal-edge recovery are non-substitutable downstream tasks.

\item \textbf{An estimand-matched, cell-split positive control for what real Perturb-seq does and does not recover.} On a total-effect (reachability) ground truth---edge $A\!\to\!B$ iff $B$ shifts under $A$-knockout (BH-FDR $\le 0.05$, $|\log_2\mathrm{FC}|\ge 0.5$), with the ground truth and the effect estimand $|\mathrm{ACE}_{ij}|=|\mathbb{E}[X_j\mid\mathrm{do}(X_i)]-\mathbb{E}[X_j\mid\mathrm{ctrl}]|$ computed on \emph{disjoint} cell halves (\S\ref{sec:a3-localization})---real Perturb-seq clears a column-shuffle null by $2$--$3\times$ ($F_1\approx0.43$ vs $0.17$). But this certifies differential-expression \emph{test--retest reliability}, not causal discovery: a plain DE $t$-test recovers the \emph{same} ground truth better ($F_1\approx0.75$) than the $|\mathrm{ACE}|$ estimand ($\approx0.43$). Against a direct-edge ground truth (transitive reduction), real recovery is near chance ($F_1\approx0.03$ vs a $\approx0.03$ null). Real interventional data thus reproducibly recovers a total-effect / reachability map, but leaves direct causal structure near-unidentifiable at achievable SNR.

\item \textbf{The substitutability verdict and its mechanism.} No VCM reaches real-data quality---even on the generous, recoverable total-effect channel. The mechanism is marginal-mean fidelity, not joint-structure blindness: a perfect-mean emitter that overlays the \emph{real} per-gene means ties real ($F_1\approx0.42$ vs $0.43$), so the recoverable signal is carried by the marginal means; yet mode-collapsed VCMs, whose predicted effect sizes are inflated $\sim\!23\times$~\citep{mejia2025diversity}, fall $3$--$5\times$ below real ($F_1\approx0.08$--$0.18$) on that same trivial channel. VCMs fail because their means are wrong, not because causal recovery needs joint structure the mean-overlay evaluation cannot see.

\item \textbf{Honest scoping, and an invariance result demoted to a corollary.} We scope the causal claim precisely (\S\ref{sec:scope}): the recovered object is a total-effect / differential-expression map, not a direct causal graph. Accordingly, the fact that the mean-overlay evaluation is invariant to a prediction's joint structure (Lemma~\ref{lem:overlay-invariance}) is retained only as a corollary of published covariance-fidelity results~\citep{riasat2026marginals,rubbi2026ggeval}, and is explicitly \emph{not} the reason VCMs fail here---the perfect-mean tie shows the recoverable signal already lives in the means.

\item \textbf{Open benchmark and code.} We release the full pipeline, fixed gene lists, seeds, and per-backend real-data ground truths for both the total-effect and direct-edge estimands, so the substitutability question can be re-asked of any future VCM.
\end{enumerate}
...
\paragraph{Downstream-utility benchmarks for VCMs.} A concurrent shift reframes VCM evaluation around \emph{downstream utility} rather than per-gene reconstruction. Most directly, \citet{bereket2026perturbhd} argue that abstract perturbation metrics fail to measure whether a VCM is actually useful for scientific discovery, and propose a benchmark that scores models on downstream \emph{hit discovery}---ranking which perturbations yield a phenotype of interest. PerturbCausal shares this utility-first premise but targets an orthogonal, non-substitutable downstream: where PerturbHD measures utility for hit \emph{ranking}, we measure utility for \emph{causal-structure} recovery---whether predicted Perturb-seq can replace real Perturb-seq as the input to a fixed causal-discovery algorithm. The two are not reducible to one another: a model can rank hits well while distorting the gene-gene structure a discovery algorithm consumes, and conversely can preserve conditional-independence structure while mis-ranking magnitudes, so neither benchmark subsumes the other. The connection is tight rather than incidental---J.\ Leskovec is a senior author of both PerturbHD and CausalBench~\citep{chevalley2025causalbench}, the interventional benchmark whose real-data causal-discovery ground truth PerturbCausal builds directly on.
...
\paragraph{The recoverable signal is the marginal means, not hidden joint structure.}
Our earlier framing attributed the substitutability gap to a joint-distribution residual that a mean-only evaluation cannot reach, and motivated a covariance-matching training objective. A matched-estimand positive control (\S\ref{sec:a3-localization}) overturns that mechanism. On a perturbation-anchored \emph{total-effect} ground truth (edge $A\!\to\!B$ iff $B$ shifts under $A$-KO at BH-FDR ${\le}\,0.05$ and $|\log_2\mathrm{FC}|\,{\ge}\,0.5$), scored on a held-out cell half against an $|\mathrm{ACE}|$ ranking ($\mathrm{ACE}=\mathbb{E}[X\mid \mathrm{do}\,i]-\mathbb{E}[X\mid\mathrm{ctrl}]$) estimated on the disjoint half over $10$ seeds, an oracle emitter that overlays the \emph{real} per-gene perturbation means---a perfect-mean predictor whose cell-level joint structure is destroyed by the overlay---ties real Perturb-seq ($F_1 = 0.422$ vs $0.433$ on the $200$-gene panel; $0.287$ vs $0.303$ on the VCM panel). Were the recoverable signal in the covariance the overlay discards, this perfect-mean emitter would fall short; it does not. \textbf{The recoverable signal \emph{is} the marginal means.} The three deep VCMs, by contrast, score $F_1 = 0.08$--$0.18$ against real's $0.30$--$0.43$ on this same channel---a $3$--$5\times$ deficit---not because they miss joint structure, but because their means are wrong: the mode collapse and ${\sim}23\times$ effect-size inflation documented in \S\ref{sec:reconciliation} corrupt precisely the marginal this channel rewards. The VCMs fail the \emph{generous} (mean-only, reachability) test, so no appeal to inaccessible joint structure is required to explain non-substitutability, and this is not an inversion---the deep models do not tie real, they fall well below it.

\paragraph{What real data recovers there is differential-expression reliability, not causal structure.}
This positive control is a reliability floor, not causal discovery. The ground truth and the $|\mathrm{ACE}|$ estimand are two thresholdings of one object---the marginal perturbation shift $\Delta(A,B)$---so real $\gg$ null certifies only that the perturbation-effect matrix is \emph{reproducible across disjoint cell halves} (split-half reliability), a property of any heterogeneous, reproducible measurement, causal or not. Two diagnostics make this concrete. First, a plain differential-expression $t$-test---ranking $(A,B)$ by $|\text{Welch-}t|$, the statistic that \emph{defines} the ground truth---recovers it \emph{better} than the matched causal $|\mathrm{ACE}|$ estimand ($0.60$--$0.75$ vs $0.30$--$0.43$ at every stratum), so what is recovered is a reachability / total-effect (differential-expression) map, not a causal graph. Second, direct causal structure---the transitive reduction of the total-effect set---is near-unidentifiable at achievable SNR: real recovers only $F_1 = 0.03$--$0.12$, roughly $10\times$ below the reachability signal and only marginally above a $0.02$--$0.05$ permutation null, while every VCM sits at or below that direct-edge null. This is the honest scope of the causal claim: the benchmark cleanly separates substitutable from non-substitutable predictors on the recoverable (total-effect) channel, whereas direct-edge orientation lies beyond the identifiability of both real and predicted data at this scale.

\paragraph{The overlay-invariance lemma is a corollary, not the mechanism.}
This reframing demotes---without contradicting---Lemma~\ref{lem:overlay-invariance} and the covariance-objective null (\S\ref{sec:calib-sweep}, \S\ref{app:j2}). The lemma is correct: the mean-overlay reduces any prediction to its per-gene mean before an edge is scored, so a covariance penalty cannot register and the observed null is expected. But because the recoverable signal already lives in the means---the perfect-mean emitter ties real---the metric's blindness to joint structure is \emph{consistent with}, not the \emph{cause of}, the failure. A VCM that emitted correct means would substitute; the deep models do not because their means are wrong. We therefore retain the lemma only as a benchmark-specific corollary of the general covariance-fidelity result of \citet{riasat2026marginals}---matching all marginals leaves the covariance unconstrained---and of the gene-expression generative-evaluation framework of \citet{rubbi2026ggeval}, used to scope what the mean-overlay channel can and cannot see, and we withdraw the earlier claim that the missing signal is localized to cell-level joint structure.
...
We claim no novelty for this invariance: it is the discovery-harness specialization of the general fact that matching all marginals leaves the joint covariance unconstrained and can flip covariance-dependent downstream estimators~\citep{riasat2026marginals}, instantiated for gene-expression generative evaluation~\citep{rubbi2026ggeval}, with Lemma~\ref{lem:overlay-invariance} contributing only the explicit reduction to the per-gene ratio vector $r$ for \emph{this} mean-overlay pipeline. We accordingly treat it descriptively---as a statement of which structure the harness discards---rather than as the reason the VCMs evaluated here underperform, since their per-gene means are themselves inaccurate (\S\ref{sec:a3-localization}) and the overlay does register mean errors.
...
\subsection{What real Perturb-seq recovers: a matched-estimand positive control}
\label{sec:a3-localization}
The mean-channel analysis above (Lemma~\ref{lem:overlay-invariance}, the calibration null) is negative. It leaves open the constructive question a substitutability benchmark must answer against a \emph{non-circular} ground truth: what does real Perturb-seq itself reproducibly recover, and is that object causal structure? We pre-registered this test, and an adversarial falsification battery, before running either (\texttt{prereg\_positive\_control.md}, Amendment~2). On Replogle K562 we build a perturbation-anchored \emph{reachability} ground truth (GT)---edge $A\to B$ iff $B$ shifts under $A$-knockout (Welch $t$ vs.\ control, BH-FDR $q\le0.05$, $|\log_2\mathrm{FC}|\ge0.5$)---and score an estimand \emph{matched} to it: the thresholded total-effect ranking $|\mathrm{ACE}_{ij}|$, with $\mathrm{ACE}_{ij}=\mathbb{E}[X_j\mid\mathrm{do}(i)]-\mathbb{E}[X_j\mid\mathrm{ctrl}]$. To break the tautology of scoring an estimand against a thresholded copy of itself on the same cells, we \emph{cross-split}: the GT is built on one held-out cell-half and every effect estimate on the disjoint half (10 fresh seeds; permutation null by column-shuffle of the effect matrix). Four pre-registered gates vary the panel and perturbation-selection stratum: S0 (baseline), S1 (strong-perturbation), S4 (strong$\,\cap\,$high-coverage), and SVCM (the VCM-predicted panel); see Table~\ref{tab:posctrl}.

Real Perturb-seq clears the null by $2$--$3\times$ on the total-effect GT ($F_1 = 0.30$--$0.43$ vs.\ null $0.10$--$0.17$; Table~\ref{tab:posctrl}). Taken alone this reads as a working positive control. The pre-registered falsification battery shows it is a \emph{reliability coefficient, not causal discovery}. A plain two-sample differential-expression (DE) test---ranking $(A,B)$ by $|\text{Welch-}t\text{ of }B\text{ under }A\text{-KO}|$, the very statistic that \emph{defines} the GT---recovers the same graph \emph{better} than the matched causal estimand ($F_1 = 0.60$--$0.75$ vs.\ $0.30$--$0.43$) at every gate. The estimand and the GT are two thresholdings of a single object, the marginal perturbation-effect matrix $\Delta(A,B)$; real $\gg$ null therefore certifies only that this effect matrix is \emph{reproducible across disjoint cell halves}---split-half reliability, which holds for any heterogeneous, reproducibly measured quantity, causal or not. What real Perturb-seq recovers here is a total-effect / reachability (differential-expression) map, not a causal graph.

Direct causal structure is near-unidentifiable at this signal-to-noise. Scored against a direct-edge GT (the transitive reduction of the total-effect set), real $F_1$ collapses to $0.03$--$0.12$---an order of magnitude below the reachability signal and only marginally above a $0.02$--$0.05$ permutation null (Table~\ref{tab:posctrl}, right)---and every VCM sits at or below the direct-edge null at every gate. This is the honest scope of the causal claim: real Perturb-seq reproducibly recovers reachability but recovers \emph{direct} regulatory edges barely above chance. (The transitive reduction is an imperfect direct-edge proxy on a cyclic network, and TRRUST-curated in-panel edges were too sparse to serve as an independent gold standard; \S\ref{sec:limitations}.)

The substitutability verdict survives on the recoverable channel, and its mechanism is now pinned down. A perfect-mean emitter---the overlay of the \emph{real} per-gene means through the same sampler---ties real on the total-effect GT (overlay $0.29$--$0.42$ vs.\ real $0.30$--$0.43$; Table~\ref{tab:posctrl}). The recoverable signal therefore \emph{is} the marginal means: on this channel there is no hidden joint-structure advantage for real cells to exploit. Yet the three deep VCMs score well below both real and overlay at every gate (best-VCM $F_1 = 0.03$--$0.19$; a $\sim\!2$--$12\times$ deficit), because their mode-collapsed, $\sim\!23\times$-inflated means (\S\ref{sec:reconciliation}) are simply the \emph{wrong} means---they fail even this generous, mean-only reachability channel. Two consequences follow. First, no VCM substitutes for real Perturb-seq, and the failure is localized to marginal-mean fidelity, not to joint structure the metric hides; this is not an inversion---predicted does not tie real. Second, the mean-overlay invariance (Lemma~\ref{lem:overlay-invariance}) is \emph{not} the operative reason VCMs fail here, since a perfect-mean model reaches real quality and pays nothing for the metric's blindness to joint structure. We accordingly retain the invariance only as a corollary of published covariance-fidelity results \citep{riasat2026marginals,rubbi2026ggeval}, not as the mechanism of the substitutability gap. What real Perturb-seq recovers is a reproducible differential-expression map---a utility question complementary to downstream hit-discovery benchmarks \citep{bereket2026perturbhd}---and current VCMs cannot reproduce even that. Full grid, null bands, and the locked falsification predictions: \texttt{prereg\_positive\_control.md} (Amendment~2), \texttt{results/positive\_control/pc\_grid.json}.

\begin{table*}[t]
\vskip 0.1in
\caption{Matched-estimand positive control on Replogle K562 (10 seeds, cross-split). Edge-set $F_1$ against a perturbation-anchored ground truth (GT). \emph{Left:} total-effect / reachability GT (edge $A\to B$ iff $B$ shifts under $A$-KO). \textbf{Real} scores real Perturb-seq under the matched total-effect estimand $|\mathrm{ACE}|$; \textbf{DE\,$t$} is a plain two-sample $t$-test on the same data; \textbf{Overlay} is the perfect-mean overlay of the real per-gene means; \textbf{VCM$_{\text{best}}$} is the best of CPA/GEARS/Geneformer; \textbf{Null} is the column-shuffle permutation null ($97.5$th percentile). \emph{Right:} direct-edge GT (transitive reduction of the total-effect set), real vs.\ its null. Gates are pre-registered perturbation strata (\texttt{prereg\_positive\_control.md}). The DE $t$-test outscoring the matched causal estimand on every total-effect gate, and real collapsing to the direct-edge null, are the pre-committed falsification outcomes (Amendment~2).}
\label{tab:posctrl}
\centering
\small
...
\toprule
 & & \multicolumn{5}{c}{\textbf{Total-effect (reachability) GT}} & \multicolumn{2}{c}{\textbf{Direct-edge GT}} \\
\cmidrule(lr){3-7}\cmidrule(lr){8-9}
\textbf{Gate} & \textbf{Panel} & \textbf{Real} & \textbf{DE\,$t$} & \textbf{Overlay} & \textbf{VCM$_{\text{best}}$} & \textbf{Null} & \textbf{Real} & \textbf{Null} \\
\midrule
S0 (baseline)          & 200 & 0.433 & 0.754 & 0.422 & 0.094 & 0.173 & 0.034 & 0.026 \\
S1 (strong-pert)       & 200 & 0.374 & 0.661 & 0.355 & 0.030 & 0.105 & 0.031 & 0.016 \\
S4 (strong$\cap$cov)   & 117 & 0.355 & 0.703 & 0.338 & 0.042 & 0.124 & 0.044 & 0.025 \\
SVCM (VCM panel)       & 176 & 0.303 & 0.601 & 0.287 & 0.185 & 0.135 & 0.115 & 0.052 \\
\bottomrule
\end{tabular}
\vskip -0.05in
\end{table*}
...
PerturbCausal is the first benchmark to ask whether virtual-cell-model (VCM)--predicted Perturb-seq can substitute for \emph{real} Perturb-seq as the input to causal gene-regulatory-network recovery, holding the discovery pipeline fixed and varying only the data source. The answer is negative, and it does not depend on how strictly the causal target is drawn. Across four VCMs (CPA, GEARS, Geneformer, and the 2025 Arc Institute STATE model), four discovery backends, and three datasets spanning cell context and CRISPR modality (K562, RPE1, Norman), no VCM reaches real-data quality, and STATE's single-seed apparent advantage does not replicate. The verdict \emph{strengthens} under the most forgiving version of the task. On a total-effect (reachability) target---the easiest recoverable channel, where an edge $A\!\to\!B$ is a thresholding of the marginal shift $A$ induces on $B$ ($|\mathrm{ACE}|$ ranked against a perturbation-anchored, BH-FDR reachability graph on a disjoint cell half)---a perfect-mean emitter that overlays the real per-perturbation means \emph{ties} real Perturb-seq ($F_1 \approx 0.42$ vs.\ $0.43$), yet the mode-collapsed VCMs, whose predicted effect sizes are inflated $\approx\!23\times$ and nearly identical across perturbations, score three-to-fivefold below real ($0.08$--$0.18$ vs.\ $0.30$--$0.43$). VCMs thus fail even the generous channel a perfect predictor passes; the substitutability headline holds a fortiori.

The same total-effect analysis corrects the mechanism and sharpens the honest scope of the causal claim. First, what real Perturb-seq recovers on this benchmark is a reachability / differential-expression map, not a causal graph: real clears a column-shuffle null by roughly $2.5\times$ ($F_1 \approx 0.43$ vs.\ $0.17$), but a plain differential-expression Welch $t$-test recovers the \emph{same} ground truth \emph{better} ($0.60$--$0.75$) than the $|\mathrm{ACE}|$ total-effect estimand ($0.30$--$0.43$)---the two are thresholdings of one object (the perturbation-effect matrix), and the ``real $\gg$ null'' signal certifies that this matrix is reproducible across disjoint cell halves, i.e.\ DE test--retest reliability, not recovered causal structure. Second, \emph{direct} causal structure is near-unidentifiable at achievable single-cell SNR for real and predicted data alike: scored against a direct-edge ground truth (transitive reduction of the total-effect set), real reaches only $F_1 = 0.03$--$0.12$, about tenfold below the reachability signal and only marginally above a $0.02$--$0.05$ permutation null, with every VCM at or below that null. Third, VCMs fail because their marginal means are wrong (mode collapse), \emph{not} because causal discovery needs joint structure the mean-overlay evaluation hides: because a perfect-mean overlay ties real, the recoverable signal \emph{is} the marginal means, and VCMs miss it by predicting the wrong ones rather than by discarding covariance. Accordingly, the mean-overlay invariance property (Lemma~\ref{lem:overlay-invariance}) is retained only as a corollary of published covariance-fidelity results~\citep{riasat2026marginals,rubbi2026ggeval} and is explicitly \emph{not} the reason VCMs fail here. This complements rather than duplicates PerturbHD~\citep{bereket2026perturbhd}, which shows abstract perturbation metrics fail to predict VCM utility for hit discovery: hit ranking and causal-edge recovery are non-substitutable downstreams, and PerturbCausal supplies the causal-structure axis. The benchmark, the total-effect/direct-edge decomposition, the multi-seed cross-split pipeline, and the reproducibility infrastructure are released as the open-source \texttt{PerturbCausal} package.

\paragraph{Limitations of the causal target.}
Three caveats bound the causal claim. \emph{Direct-edge ground truth.} We obtain the direct-edge target by transitive reduction of the total-effect set, which is uniquely defined only for acyclic graphs; on the cyclic feedback structure of real regulatory networks it can misclassify direct versus indirect edges, so the near-zero direct-edge $F_1$ is a conservative estimate of direct-structure recoverability rather than an exact one. \emph{External validation density.} The literature-curated TRRUST~v2 network~\citep{han2018trrust} is too sparse on the essential-gene panels used here to validate the recovered ground truth against known biology at adequate power (\S\ref{sec:limitations}); the reachability-versus-causal distinction we draw is therefore internal to the perturbation-anchored target, not adjudicated against an external gold standard. \emph{Magnitudes.} The $F_1$ values reported for the total-effect analysis come from a cell-capped, BH-corrected re-analysis and are intended for within-benchmark contrasts (real vs.\ null vs.\ perfect-mean vs.\ VCM); they are not calibrated to, and should not be compared numerically against, the committed genome-scale screen, whose absolute magnitudes differ. These caveats bound internal validity; the qualitative ordering---real $>$ VCM on every channel, and direct structure near-unidentifiable for real and predicted alike---is robust to each.
...
\bibitem[Bereket \& Leskovec(2026)]{bereket2026perturbhd}
Bereket, M. and Leskovec, J.
\newblock Are current {AI} virtual cell models useful for scientific discovery?
\newblock \emph{bioRxiv 2026.04.23.719015}, 2026.

\bibitem[Riasat(2026)]{riasat2026marginals}
Riasat, N.
\newblock When marginals match but structure fails: Covariance fidelity in generative models.
\newblock \emph{arXiv preprint arXiv:2603.17041}, 2026.

\bibitem[Rubbi et al.(2026)]{rubbi2026ggeval}
Rubbi, A., Di~Francesco, A.~G., Lotfollahi, M., and Li{\`o}, P.
\newblock A standardized framework for evaluating gene expression generative models.
\newblock \emph{arXiv preprint arXiv:2603.11244}, 2026.
